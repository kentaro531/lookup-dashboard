...
def get_snapshot_dir():
    """In CI mode, use repo-local dir. In local mode, use home dir."""
    if os.environ.get("CI") == "true":
        return Path("./snapshots")
    return Path.home() / ".lookup_snapshots"
ANTHROPIC_API = "https://api.anthropic.com/v1/messages"
SLACK_API = "https://slack.com/api"
EXCLUDE_PROJECTS = ["00_LIFE"]
ACCG_PREFIX = "40_ACCG_"
EXCLUDE_MEMBERS = ["Nao Matsumoto", "迫田昂輝", "Shihomi Okit"]
...
def build_member_data(tasks, changes, collabs, projects, sections):
    ...
    for m in members:
        ...
        long_ov = sum(1 for t in m["overdue"] if t["days_ov"] >= 7)
        postponed_count = sum(1 for t in m["overdue"] if t["postponed"])
        score = 100 - (ov_count * 1.5) - (long_ov * 0.5) - (postponed_count * 3)
...
def main():
    print("\n" + "="*50)
    print("  LOOK UP Todoist Dashboard v9")
    ...
    output_path = OUTPUT
    if is_ci():
        output_path = Path("index.html")  # Save to repo root for GitHub Pages
